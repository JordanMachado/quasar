define(['underscore','TweenMax','TimeLineLite','PIXI','app/PixiPOV','app/animations/Animation'],function(_, TweenMax, TimeLineLite, PIXI, PixiPOV, Animation){
	...
	_.extend(Perc6Animation.prototype,Animation.prototype,{
		tl:new TimelineLite(),
		buildShape: function() {
			this.shape = new PIXI.Text('TSI-ts', {font:"80px BebasNeue",fill:'#ecebeb'});
		...
		start:function() {
			...
			this.tl.to(this.shape.scale, 0.2, {x:1,y:1,ease:this.easeIn},'-=0.2');
			this.tl.to(this.shape.scale, 0.2, {x:0.7,y:0.7},'+=0.2');
